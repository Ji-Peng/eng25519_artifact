...
EVP_PKEY *EVP_PKEY_keygen_wrapper(int nid, int arg2, ENGINE *engine)
{
    ...
    switch (nid) {
        ...
        case EVP_PKEY_DSA:
            ...
            if (!EVP_PKEY_CTX_set_dsa_paramgen_bits(pctx, arg2)) {
                BIO_printf(bio_err, "%s: Failure in setting key bits: %d\n",
                           OBJ_nid2sn(nid), arg2);
                goto evp_keygen_err;
            }
            break;
    ...
    /* RSA keys set the key length during key generation rather than parameter
     * generation! */
    if (nid == EVP_PKEY_RSA) {
...
int EVP_PKEY_name_parser(int *_nid, int *_subparam, ENGINE **_e,
                         const char *name)
{
    int rt = 0;
    int nid = NID_undef;
    ...
    if (len > 12 && !strncmp(name, "EVP_PKEY_EC:", 12)) {
        ...
        if (nid == NID_undef) {
            BIO_printf(bio_err, "%s is an unknown algorithm\n", name);
            goto end;
        }
