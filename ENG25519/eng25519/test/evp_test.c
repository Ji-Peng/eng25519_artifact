...
static int check_test_error(struct evp_test *t)
{
    ...
    if (func == NULL && reason == NULL) {
        fprintf(stderr,
                "Test line %d: expected error \"%s:%s\", no strings available. "
                " Skipping...\n",
                t->start_line, t->func, t->reason);
        return 1;
    }
...
static int check_output(struct evp_test *t, const unsigned char *expected,
                        const unsigned char *received, size_t len)
{
...
#    ifndef OPENSSL_V102_COMPAT
    OPENSSL_init_crypto(OPENSSL_INIT_LOAD_CONFIG | OPENSSL_INIT_ENGINE_DYNAMIC,
                        NULL);
#    else
    OPENSSL_config(NULL);
#    endif /* OPENSSL_V102_COMPAT */
...
static int digest_test_parse(struct evp_test *t, const char *keyword,
                             const char *value)
{
...
static int cipher_test_enc(struct evp_test *t, int enc, size_t out_misalign,
                           size_t inp_misalign, int frag)
{
    struct cipher_data *cdat = t->data;
    ...
    if (cdat->iv) {
        if (cdat->aead) {
            if (!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, cdat->iv_len,
                                     0))
                goto err;
    ...
    if (cdat->aead) {
        ...
        if (tag || cdat->aead != EVP_CIPH_GCM_MODE) {
            if (!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, cdat->tag_len,
                                     tag))
                goto err;
        }
    }
    ...
    if (cdat->aad) {
        ...
        if (!frag) {
            ...
            if (cdat->aad_len > 1 &&
                !EVP_CipherUpdate(ctx, NULL, &chunklen, cdat->aad + donelen, 1))
                goto err;
        }
    }
    ...
    if (enc && cdat->aead) {
        ...
        if (!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, cdat->tag_len,
                                 rtag)) {
...
static int mac_test_parse(struct evp_test *t, const char *keyword,
                          const char *value)
{
 ...
 */
static int pkey_test_init(struct evp_test *t, const char *name, int use_public,
...
static int pkey_test_parse(struct evp_test *t, const char *keyword,
                           const char *value)
{
    struct pkey_data *kdata = t->data;
    if (strcmp(keyword, "Input") == 0)
...
static int pkey_test_run(struct evp_test *t)
{
    ...
    err = "KEYOP_ERROR";
    if (kdata->keyop(kdata->ctx, out, &out_len, kdata->input,
                     kdata->input_len) <= 0)
        goto err;
...
{
    return pkey_test_init(t, name, 0, EVP_PKEY_decrypt_init, EVP_PKEY_decrypt);
}
...
static int pderive_test_parse(struct evp_test *t, const char *keyword,
                              const char *value)
{
    struct pkey_data *kdata = t->data;
...
#    ifndef OPENSSL_NO_SCRYPT
static int scrypt_test_parse(struct evp_test *t, const char *keyword,
                             const char *value)
{
...
static int pbkdf2_test_parse(struct evp_test *t, const char *keyword,
                             const char *value)
{
    struct pbe_data *pdata = t->data;
...
static int pkcs12_test_parse(struct evp_test *t, const char *keyword,
                             const char *value)
{
    struct pbe_data *pdata = t->data;
...
static int pbe_test_parse(struct evp_test *t, const char *keyword,
                          const char *value)
{
    struct pbe_data *pdata = t->data;
...
static int pbe_test_run(struct evp_test *t)
{
    ...
    if (pdata->pbe_type == PBE_TYPE_PBKDF2) {
        err = "PBKDF2_ERROR";
        if (PKCS5_PBKDF2_HMAC((char *)pdata->pass, pdata->pass_len, pdata->salt,
                              pdata->salt_len, pdata->iter, pdata->md,
                              pdata->key_len, key) == 0)
            goto err;
#    ifndef OPENSSL_NO_SCRYPT
    } else if (pdata->pbe_type == PBE_TYPE_SCRYPT) {
        ...
        if (EVP_PBE_scrypt((const char *)pdata->pass, pdata->pass_len,
                           pdata->salt, pdata->salt_len, pdata->N, pdata->r,
                           pdata->p, pdata->maxmem, key, pdata->key_len) == 0)
            goto err;
#    endif
    } else if (pdata->pbe_type == PBE_TYPE_PKCS12) {
        err = "PKCS12_ERROR";
        if (PKCS12_key_gen_uni(pdata->pass, pdata->pass_len, pdata->salt,
                               pdata->salt_len, pdata->id, pdata->iter,
                               pdata->key_len, key, pdata->md) == 0)
            goto err;
    }
    err = "KEY_MISMATCH";
...
static int encode_test_parse(struct evp_test *t, const char *keyword,
                             const char *value)
{
    struct encode_data *edata = t->data;
    if (strcmp(keyword, "Input") == 0)
...
static int encode_test_run(struct evp_test *t)
{
    ...
    if (edata->encoding == BASE64_CANONICAL_ENCODING) {
        ...
        EVP_EncodeUpdate(encode_ctx, encode_out, &chunk_len, edata->input,
                         edata->input_len);
    ...
    if (edata->encoding != BASE64_INVALID_ENCODING &&
        check_var_length_output(t, edata->input, edata->input_len, decode_out,
                                output_len)) {
        err = "BAD_DECODING";
        goto err;
    }
...
static int kdf_test_parse(struct evp_test *t, const char *keyword,
                          const char *value)
{
    struct kdf_data *kdata = t->data;
    if (strcmp(keyword, "Output") == 0)
 ...
 *      return 0 if called
 */
static int void_test_parse(struct evp_test *t, const char *keyword,
                           const char *value)
{
    (void)t;
    (void)keyword;
