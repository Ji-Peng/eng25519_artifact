...
#include "providers/batch_fast_lib/x25519_batch_avx512ifma/utils.h"
...
void point_encoding_1x8w_ed25519(uint8_t *encP, const PointXYTZ_2x4w *P)
{
    ...
    encP[ED25519_KEY_SIZE - 1] =
        (encP[ED25519_KEY_SIZE - 1] & 0x7F) | ((encX[0] & 0x1) << 7);
}

const uint64_t param_curve_d[NUM_DIGITS_fp255_X64] = {
    0x75eb4dca135978a3, 0x00700a4d4141d8ab, 0x8cc740797779e898, 0x52036cee2b6ffe73};
const uint64_t one[NUM_DIGITS_fp255_X64] = {1, 0, 0, 0};
...
{
    uint64_t X[NUM_DIGITS_fp255_X64];
    uint64_t Y[NUM_DIGITS_fp255_X64];
    uint64_t T[NUM_DIGITS_fp255_X64];
    uint64_t u[NUM_DIGITS_fp255_X64];
    uint64_t v[NUM_DIGITS_fp255_X64];
