...
if (UNIX)
  # QUIET表示不显示查询过程中产生的消息
  find_package(PkgConfig QUIET)
  # 去默认路径下找openssl模块的库，找到后设置如下变量：_OPENSSL_FOUND, _OPENSSL_LIBRARIES, _OPENSSL_VERSION等变量
...
if (WIN32)
  ...
  # 设置变量，_OPENSSL_ROOT_HINTS = ${OPENSSL_ROOT_DIR}+环境变量OPENSSL_ROOT_DIR
  set(_OPENSSL_ROOT_HINTS
    ${OPENSSL_ROOT_DIR}
    ENV OPENSSL_ROOT_DIR
    )
...
# 该命令结果存储在OPENSSL_INCLUDE_DIR中
# 搜索的对象是openssl/ssl.h、${_OPENSSL_ROOT_HINTS_AND_PATHS}
# HINTS用于指定除默认路径外的搜索路径
# PATH_SUFFIXES指定需额外搜索的子路径
...
if(WIN32 AND NOT CYGWIN)
  ...
  # 用于寻找静态/动态库，最终库的名字存储在OPENSSL_SSL_LIBRARY变量中
  # NAMES_PER_DIR: 在指定路径中搜索所有名字
  # HINTS：指定除默认位置外的搜索路径
  # PATH_SUFFIXED：额外搜索的子目录
...
if(OPENSSL_INCLUDE_DIR AND EXISTS "${OPENSSL_INCLUDE_DIR}/openssl/opensslv.h")
  ...
  file(STRINGS "${OPENSSL_INCLUDE_DIR}/openssl/opensslv.h" openssl_version_pre_release_str
      REGEX "^#[\t ]*define[\t ]+OPENSSL_VERSION_PRE_RELEASE[\t ]+\"([^\"]+)\".*")
  ...
  if(openssl_version_major_str AND openssl_version_minor_str AND openssl_version_patch_str AND openssl_version_pre_release_str)
      ...
      string(REGEX REPLACE "^.*OPENSSL_VERSION_PRE_RELEASE[\t ]+\"([^\"]+)\".*$"
          "\\1" OPENSSL_VERSION_PRE_RELEASE_STRING "${openssl_version_pre_release_str}")

      set(OPENSSL_VERSION "${OPENSSL_VERSION_MAJOR}.${OPENSSL_VERSION_MINOR}.${OPENSSL_VERSION_PATCH}${OPENSSL_VERSION_PRE_RELEASE_STRING}")

  elseif(openssl_version_str)
